...
month_count = a %>% 
  group_by(month) %>%
  summarise('total_month' = length(total)) %>%
  filter(total_month >5) 

#layering the ST_count with at least 5 in the initial filter of a with total equalling 10
a = filter(a, month %in% month_count$month)
...
plot_month <- 
  ...
  labs(title="Resistance isolates (Isoleucine) changes in human stool samples overtime in different months \nRed line shows when the growth promoting antibiotic were banned in 2006 in the UK",
       x ="Year", y = "Log scale proportion") +
  theme(plot.title = element_text(lineheight=.8, face="bold",hjust = 0.5))

plot_month

ggsave("/Users/user/Documents/OneDrive - Nexus365/PhD/Campy_Analysis_ALL/Figures/Scatterplot/plots_month.png",plot=plot_month, width = 50, height = 30, units = "cm",dpi = 600)
