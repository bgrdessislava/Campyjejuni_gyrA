...
year_df<- read.csv("/Users/user/Documents/OneDrive - Nexus365/PhD/Campy_Analysis_ALL/Data/CC45_859isolate_df_allMeta_toUSE.csv")
...
split_plot <- ggplot(aes(year_df$year, year_df$binary), data = year_df) + 
  ...
  xlab("year") + 
  ylab("resistance") +
  ggtitle("CC45 Resistance oveer ST types")

split_plot

ggsave("/Users/user/Documents/OneDrive - Nexus365/PhD/Campy_Analysis_ALL/Figures/Scatterplot/binary_CC45_STtype.png", plot=split_plot,dpi = 600)

#region
split_plot_sample <- ggplot(aes(year_df$year, year_df$binary), data = year_df) + 
  geom_point() + 
  geom_smooth(method = glm, method.args = c(family=binomial)) +
  facet_wrap(~ year_df$region_Tableau) + # create a facet for each mountain range
  xlab("year") + 
  ylab("resistance") +
  ggtitle("CC45 Resistance over time across region")


split_plot_sample 
#split_plot_ST dont run too many
ggsave("/Users/user/Documents/OneDrive - Nexus365/PhD/Campy_Analysis_ALL/Figures/Scatterplot/binary_CC_45region.png",plot=split_plot_sample,dpi = 600)
...
#Mix model looking at ST and having the year as random effect
mixed.lmer_ST_difference <- lmer(year_df$binary ~ year_df$ST..MLST. + (1|year_df$year), data = year_df)
...
summary(mixed.lmer_ST_difference)
