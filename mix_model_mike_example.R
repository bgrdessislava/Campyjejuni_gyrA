...
#clonal_complex each of them shown as whether it is significant or not
clonal.lm <- lm(year_df$binary ~ year_df$year + year_df$clonal_complex, data = year_df)
summary(clonal.lm)

#Mix model inluding the clonal complex
mixed.lmer <- lmer(year_df$binary ~ year_df$year + (1|year_df$clonal_complex), data = year_df)
